...
class VaultTransferRepositoryImpl @Inject constructor(
    ...
    override suspend fun createLocalBackup(vaultId: String): BackupRecordSummary {
        ...
        val file = File(backupDir(), "vault-${vaultId}-${now}.ocbak")
    ...
    override suspend fun exportContactsJson(vaultId: String): ImportExportHistorySummary {
        ...
        val file = File(exportDir(), "contacts-${vaultId}-${now}.json")
        val payload = JSONObject().apply {
            put("vaultId", vaultId)
            put("createdAt", now)
    ...
    override suspend fun exportContactsCsv(vaultId: String): ImportExportHistorySummary {
        ...
        val now = System.currentTimeMillis()
        val contacts = dao.getAllDetailed().map { it.toModel() }
        val file = File(exportDir(), "contacts-${vaultId}-${now}.csv")
    ...
    override suspend fun exportContactsVcf(vaultId: String): ImportExportHistorySummary {
        ...
        val now = System.currentTimeMillis()
        val contacts = dao.getAllDetailed().map { it.toModel() }
        val file = File(exportDir(), "contacts-${vaultId}-${now}.vcf")
    ...
    override suspend fun exportContactsExcel(vaultId: String): ImportExportHistorySummary {
        ...
        val now = System.currentTimeMillis()
        val contacts = dao.getAllDetailed().map { it.toModel() }
        val file = File(exportDir(), "contacts-${vaultId}-${now}.xls")
    ...
    private suspend fun importSummaries(vaultId: String, contacts: List<ContactSummary>, operation: String, filePath: String): ImportExportHistorySummary {
        ...
        contacts.forEach { summary ->
            ...
            summary.folderName?.takeIf { it.isNotBlank() }?.let { dao.upsertFolder(FolderEntity(it, it, "folder", "blue", now)) }
    ...
    private fun backupDir(): File = File(context.filesDir, "vault_backups").apply { mkdirs() }
    private fun exportDir(): File = File(context.filesDir, "vault_exports").apply { mkdirs() }
...
private fun FolderEntity.toJson(): JSONObject = JSONObject().apply { put("folderName", folderName); put("displayName", displayName); put("iconToken", iconToken); put("colorToken", colorToken); put("createdAt", createdAt) }
...
private fun ContactSummary.toJson(): JSONObject = JSONObject().apply { put("id", id); put("displayName", displayName); put("primaryPhone", primaryPhone); put("tags", JSONArray(tags)); put("isFavorite", isFavorite); put("folderName", folderName) }
...
private fun JSONArray.toFolderEntities(): List<FolderEntity> = (0 until length()).map { i -> getJSONObject(i).let { FolderEntity(it.getString("folderName"), it.getString("displayName"), it.optString("iconToken", "folder"), it.optString("colorToken", "blue"), it.optLong("createdAt")) } }
...
private fun JSONArray.toContactSummaries(): List<ContactSummary> = (0 until length()).map { i ->
    ...
    ContactSummary(obj.optString("id", UUID.randomUUID().toString()), obj.getString("displayName"), obj.optString("primaryPhone").takeIf { it.isNotBlank() }, obj.optJSONArray("tags")?.let { arr -> (0 until arr.length()).map { idx -> arr.getString(idx) } }.orEmpty(), obj.optBoolean("isFavorite", false), obj.optString("folderName").takeIf { it.isNotBlank() })
}
